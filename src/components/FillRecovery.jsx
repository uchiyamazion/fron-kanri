...
export function FillRecovery({ db, addRecord, deleteRecord, toast, onNavigate }) {
  ...
  const TechSelect = ({ value, onChange }) => (
    <select value={value} onChange={onChange} style={{ width: '100%' }}>
  ...
  const vendorName = id => db.vendors.find(v => v.id === id)?.name || ''

  return (
    <div>
      {(db.vendors.length === 0 || db.technicians.length === 0) && (
        <div style={{ background: '#FFF8E1', border: '0.5px solid #E8D9A0', borderRadius: 10, padding: '10px 14px', marginBottom: 12, fontSize: 12, color: '#8a6d1a' }}>
          「業者・技術者」ページで充填回収業者・技術者を登録すると、ここで選択できるようになります。
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        ...
        <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,.1)', borderRadius: 12, padding: 14 }}>
          ...
          <div style={{ marginBottom: 10 }}><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 4 }}>充填業者</label><VendorSelect value={fill.vendorId} onChange={setF('vendorId')} /></div>
          <div style={{ marginBottom: 10 }}><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 4 }}>冷媒フロン類取扱技術者</label><TechSelect value={fill.technicianId} onChange={setF('technicianId')} /></div>
        ...
        <div style={{ background: '#fff', border: '0.5px solid rgba(0,0,0,.1)', borderRadius: 12, padding: 14 }}>
          ...
          <div style={{ marginBottom: 10 }}><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 4 }}>回収業者（第一種フロン類充塡回収業者）</label><VendorSelect value={rec.vendorId} onChange={setR('vendorId')} /></div>
          <div style={{ marginBottom: 10 }}><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 4 }}>冷媒フロン類取扱技術者</label><TechSelect value={rec.technicianId} onChange={setR('technicianId')} /></div>
                  ...
                  {all.map(r => {
                    ...
                    return (
                      <tr key={r.id + r.kind} style={{ borderBottom: '0.5px solid rgba(0,0,0,.05)' }}>
                        ...
                        <td style={{ padding: '7px 8px', fontSize: 11 }}>{vendorName(r.vendorId) || '—'}</td>
