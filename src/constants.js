...
export const REPAIR_CONTENT_OPTIONS = [
  '増し締め', '異物の除去（清掃）', 'フレア部再加工', 'フレアアダプタ使用', 'ろう付け補修',
...
// 機器分類（冷媒漏えい点検・整備記録簿の「使用機器 分類」ドロップダウン準拠）
export const EQUIPMENT_CATEGORY_OPTIONS = [
  'ビル用パッケージエアコン', '店舗用パッケージエアコン', '設備用パッケージエアコン',
  'ガスヒートポンプ', 'コンデンシングユニット（ショーケース・冷蔵庫）',
  '内蔵型冷蔵ショーケース', '内蔵型業務用冷蔵庫', '冷凍冷蔵ユニット',
  '製氷機', '冷水機', '空調用チラー', 'ブラインチラー',
  '遠心式冷凍機', '輸送用冷凍冷蔵ユニット', 'その他',
]

// 機器の用途（記録簿準拠）
export const EQUIPMENT_USAGE_OPTIONS = ['冷凍用・プロセス冷却用', '空調用', 'その他']
